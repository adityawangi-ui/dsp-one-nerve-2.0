  ...
  return (
    <div className="flex gap-5">
      ...
      <div className="w-52 shrink-0">
        <div className="sticky top-0 border border-border rounded-xl bg-card p-3 space-y-0.5">
          <p className="text-xs font-semibold text-foreground mb-2">MRDR {row.mrdr}</p>
          <p className="text-[10px] text-muted-foreground truncate">{row.mrdrDescription}</p>
          <p className="text-[10px] text-muted-foreground mb-2">{lineCount} line item(s)</p>
          {sections.map(s => (
            <button
              key={s.id}
              onClick={() => scrollTo(s.id)}
              className={`w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-[11px] font-medium transition-all text-left ${
                activeSection === s.id ? "bg-primary/10 text-primary border border-primary/20" : "text-muted-foreground hover:bg-secondary"
              }`}
            >
      ...
      <div className="flex-1 min-w-0 space-y-8">
        ...
        {/* 2. Stock Info */}
        <div id="tab-insight-stock">
          <SectionHeader icon={Package} title="Stock Info / Inventory" />
          <ChartCard title="Stock Type Breakdown" className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={stockBreakdown} dataKey="value" nameKey="name" cx="50%" cy="45%" outerRadius={110} innerRadius={0}
                  label={({ name, value, cx: pcx, cy: pcy, midAngle, outerRadius: or }) => {
                    ...
                    return <text x={x} y={y} textAnchor={x > (pcx as number) ? "start" : "end"} dominantBaseline="central" fontSize={11} fontWeight={600} fill="hsl(var(--foreground))">{name} {value}%</text>;
                  }}
                  labelLine={{ stroke: "hsl(var(--muted-foreground))", strokeWidth: 1 }} strokeWidth={1} stroke="hsl(var(--background))"
                >
                ...
                <RechartsTooltip content={({ active, payload }) => {
                  ...
                  const d = payload[0];
                  return <div className="bg-card border border-border rounded-lg shadow-lg px-3 py-2 text-xs"><span className="font-semibold text-foreground">{d.name}</span><span className="text-muted-foreground ml-2">{d.value}%</span></div>;
                }} />
                <Legend verticalAlign="bottom" iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11, paddingTop: 12 }} formatter={(value: string) => <span className="text-foreground ml-1">{value}</span>} />
              </PieChart>
            </ResponsiveContainer>
          </ChartCard>
